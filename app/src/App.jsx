...
import './App.css';
import { addTabToDB, db, deleteTabDB, updateTabDB } from './db';
...
const App = () => {
  ...
  useEffect(async () => {
    ...
    async function onDeviceReady() {
      ...
      const tabsList = await db.tabsList.where('publicKey').equals(currentAlias).toArray();
  ...
  const selectKey = async (ind) => {
    ...
    if (res) {
      ...
      const tabsList = await db.tabsList.where('publicKey').equals(res.currentAlias).toArray();
      openTabsFromDB(tabsList);
    }
  }
