...
const workercode = () => {
    self.importScripts('./relay.ts');
    self.importScripts('./db.ts');
...
// eslint-disable-next-line
        ...
        async function appendTasks(tasks: ISyncTask[], taskQueue: ISyncTask[]) {
            await self.dbi.putSyncTasks(tasks)
        ...
        function updateSyncState() {
            const state = {
                ...
                totalEventCount: self.getEventsCount(),
                newEventCount,
                reload: needReload
            }
        ...
        async function processFilters(task: ISyncTask, filterTmpls: NDKFilter[], postFilter?: (e: NostrEvent) => boolean) {
            ...
            do {
                ...
                for (const ndkEvent of events.values()) {
                    ...
                    const added = self.addLocalRelayEvent(e)
                ...
                if (newEvents.length > 0) self.dbi.putLocalRelayEvents(newEvents)
...
// eslint-disable-next-line
        async function fetchLocal(filter: any): Promise<any[]> {
            ...
            // eslint-disable-next-line
            const client = new self.LocalRelayClient((msg: any) => {
        ...
        async function startSync(pubkey: string) {
            console.log('startSync', {start: 'start'})
            ...
            const newQueue = await self.dbi.listSyncTasks(pubkey)
            ...
            // last cursor
            const now = Math.floor(Date.now() / 1000)
            const lastUntil = Number((await self.dbi.getFlag(currentPubkey, 'last_until')) || '0')
            ...
            await self.dbi.setFlag(currentPubkey, 'last_until', now + '')
        ...
        // function setOnSyncState(cb: (state: ISyncState) => void) {
        //     onSyncState = cb
        // }
